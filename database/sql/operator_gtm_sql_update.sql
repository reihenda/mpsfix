-- Jika tabel belum ada, maka gunakan SQL lengkap berikut:

-- Tabel untuk menyimpan data operator GTM
CREATE TABLE IF NOT EXISTS `operator_gtm` (
...
CREATE TABLE IF NOT EXISTS `operator_gtm_lembur` (
  `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `konfigurasi_lembur` (
  `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT,
...
INSERT INTO `konfigurasi_lembur` (`nama_konfigurasi`, `tarif_per_jam`, `is_active`, `created_at`, `updated_at`) 
VALUES ('Tarif Lembur Standar', 25000.00, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE `updated_at` = NOW();

-- Jika tabel sudah ada tapi belum ada kolom sesi 3, gunakan ALTER TABLE:

-- Tambahkan kolom jam_masuk_sesi_3 jika belum ada
ALTER TABLE `operator_gtm_lembur` 
ADD COLUMN IF NOT EXISTS `jam_masuk_sesi_3` time DEFAULT NULL AFTER `jam_keluar_sesi_2`;

-- Tambahkan kolom jam_keluar_sesi_3 jika belum ada
ALTER TABLE `operator_gtm_lembur` 
ADD COLUMN IF NOT EXISTS `jam_keluar_sesi_3` time DEFAULT NULL AFTER `jam_masuk_sesi_3`;